...
/**如果是debug 模式将会把debug写入到文件 */
let DEBUG: boolean = false;

/**日志的输出目录 */
const logOutFile: string = resolve(__dirname, "../../logs/out/LogOut.log");
const logInfoFile: string = resolve(__dirname, "../../logs/info/info");
const logError: string = resolve(__dirname, "../../logs/err/err.log");
const logBasePath: string = resolve(__dirname, "../../");
...
function log(msg: string, ...args) {
  ...
  if (DEBUG) { fileLogger.debug(msg, args); }
}
...
/**错误信息写到log日志 */
function errToLog(e: Error) {
  let msg: string = e.stack || e.message;
  Log.errorLog(msg);
}
...
function initConfig(child_process_opt?: ChildProcessOpt) {
  if (child_process_opt) {
    ...
    logFileAppender.filename = resolve(basePath, `./logs/out/${child_process_opt.process_name}_${child_process_opt.process_id}_logOut.log`);
    infoFileAppender.filename = resolve(basePath, `./logs/info/info_${child_process_opt.process_name}_${child_process_opt.process_id}`);
    errorFileAppender.filename = resolve(basePath, `./logs/err/${child_process_opt.process_name}_${child_process_opt.process_id}_err.log`);
  }
...
export var Log = {
  initConfig, log, infoLog, errorLog, errToLog, DEBUG
};